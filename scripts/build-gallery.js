...
// "2026-Autumn-KonicaAutoS2-FujiC200-0024.jpg" — camera and film stock are
// always the 3rd and 4th hyphen-separated segments. Only used for the
// caption now; gallery order comes from git history (gitAddedDate) instead,
// since roll/frame numbers reset independently per camera and aren't
// comparable across different cameras sharing the same year/season.
...
function loadGitAddedDates() {
  ...
  try {
    const log = execFileSync(
      // --no-renames: git's content-similarity heuristic can otherwise pair
      // a newly-added photo with an unrelated deleted one (binary JPEGs of
      // similar size trip the default 50% threshold), reclassifying it as a
      // rename so it's silently missing from --diff-filter=A output.
      'git',
      ['log', '--no-renames', '--diff-filter=A', '--format=%x00%aI', '--name-only', '--', 'images/'],
...
async function processImages(repoSlug, branch) {
  ...
  const entries = await mapWithConcurrency(files, 8, async (file) => {
    ...
    if (!meta) {
      console.warn(`Warning: "${file}" doesn't match the expected naming pattern — it won't show a camera/film caption.`);
    }
